...
class CustomizerModelController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // ── Standard model image uploads (front/back/left/right × black/white/svg) ──
        ...
        // ── Colors (with per-color front/back/left/right views) ───
        $colorsData = [];
        try {
        ...
        foreach ($colorsData as $idx => &$color) {
            // Per-color, per-view, per-type upload: color_{idx}_{view}_{type}
            $color['views'] = [];
            foreach ($views as $view) {
                $color['views'][$view] = [];
                foreach ($types as $type) {
                    ...
                    if ($request->hasFile($fileKey)) {
                        $color['views'][$view][$type] = $request->file($fileKey)
                            ->store('models/colors', 'public');
                    } else {
                        $color['views'][$view][$type] = null;
                    }
                }
            }
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Collect old color image paths to delete orphans
        ...
        // Meta: which old paths to keep per color index
        $colorsMeta = [];
        try {
    ...
    // ─── showApi (for product-style frontend) ─────────────────────
    public function showApi($id)
    {
        ...
        return response()->json([
            'id' => $model->id,
            'name' => $model->title,   // ← yeh naya add hua
            'title' => $model->title,
...
'subcategory_id' => $model->subcategory_id,
            ...
            'thumbnail' => $model->thumbnail ? asset('uploads/models/'.$model->thumbnail) : null,  // ← yeh naya add hua
            'views' => [
                'front' => [
    ...
    public function updateOrder(Request $request)
    {
        foreach ($request->order as $item) {
            CustomizerModel::where('model_name', $item['model_name'])->update(['position' => $item['position']]);
        }
