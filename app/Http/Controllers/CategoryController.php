...
class CategoryController extends Controller
{
    ...
    {
        // ✅ Activity Log
        ActivityLogger::log('deleted', 'Category', $category->name, $category->id, [
            'name' => $category->name,
            'type' => $category->parent_id ? 'Subcategory' : 'Main Category',
        ]);
    ...
    {
        return Category::where('status', 1)->where('highlight', 1)
            ->select('id', 'name', 'icon_image', 'highlight_image')->orderBy('name')->get();
    }

    public function apiHighlighted()
    {
        return Category::where('status', 1)->where('highlight', 1)->whereNull('parent_id')
            ->select('id', 'name', 'icon_image', 'highlight_image')->orderBy('name')->get();
    }
    ...
    public function apiCategoriesByNavigation()
    {
        $categories = Category::with(['subcategories' => function ($q) {
            $q->select('id', 'name', 'parent_id', 'icon_image', 'password')->where('status', 1);
        }])->where('status', 1)->whereNotNull('navigation_id')
            ->select('id', 'name', 'navigation_id', 'icon_image', 'password')
            ->orderBy('name')->get()->groupBy('navigation_id');
    ...
    public function categoriesBySlug($slug)
    {
        ...
        $categories = Category::with(['subcategories' => function ($q) {
            $q->where('status', 1)->select('id', 'name', 'parent_id', 'icon_image');
        }])->where('navigation_id', $navigation->id)->where('status', 1)->whereNull('parent_id')
            ->select('id', 'name', 'icon_image', 'navigation_id', 'highlight_image')->get();
    ...
    {
        $navigation = Navigation::where('slug', $slug)->where('status', 1)->firstOrFail();

        $categories = Category::with(['subcategories' => function ($q) {
            $q->where('status', 1)->orderBy('position')->select('id', 'name', 'parent_id');
        }])->where('navigation_id', $navigation->id)->where('status', 1)->whereNull('parent_id')
