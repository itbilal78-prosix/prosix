<template>
  <div class="po-tab-root">
    ...
    <div class="po-tab-header">
      <div>
        <span class="po-eyebrow">Completed Orders</span>
        <h2 class="po-tab-title">My Orders</h2>
        <p class="po-tab-subtitle">
          All completed Place Orders appear here automatically.
        </p>
      </div>
    ...
    <!-- Error State -->
    <div v-else-if="errorMessage" class="po-empty">
      <h4>Unable to Load Orders</h4>
      <p>{{ errorMessage }}</p>
      <button type="button" class="po-place-btn" @click="fetchCompletedOrders">Try Again</button>
    </div>

    <!-- Empty State -->
    <div v-else-if="completedOrders.length === 0" class="po-empty">
      ...
      <p>
        When a Place Order is marked completed, it will appear here automatically.
      </p>
...
const fetchCompletedOrders = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/place-order/my-orders', {
      headers: token
    ...
    errorMessage.value = error.response?.data?.message || 'Completed orders could not be loaded.'
    orders.value = []
  } finally {
...
const getFileName = (file) => {
  ...
  if (typeof file === 'object') {
    return file.filename || file.file_name || file.name || file.path || ''
  }
  return String(file)
}
...
const getOriginalName = (file) => {
  if (!file) return 'File'
  if (typeof file === 'object') {
    return file.original || file.original_name || file.name || file.filename || file.file_name || 'File'
  }
...
const getDisplayName = (file) => {
  ...
  const cleanName = extension ? name.slice(0, -extension.length) : name
...
const getFileUrl = (file, folder) => {
  if (!file) return '#'
  if (typeof file === 'object') {
    const directUrl = file.url || file.file_url || file.full_url || file.path_url
...
const isImage = (file) => {
  const extension = getOriginalName(file).split('?')[0].split('.').pop().toLowerCase()
  return ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg'].includes(extension)
}

const getExt = (file) => {
  if (!file) return 'FILE'
  if (typeof file === 'object' && file.ext) {
    return String(file.ext).replace('.', '').toUpperCase().slice(0, 5)
  }
  const name = getOriginalName(file)
  if (!name.includes('.')) return 'FILE'
  return name.split('?')[0].split('.').pop().toUpperCase().slice(0, 5)
}
...
const hasFiles = (files) => normalizeFiles(files).length > 0
const hasAnyFiles = (order) => hasFiles(order.mockup_files) || hasFiles(order.roster_files) || hasFiles(order.quote_files)

const capitalize = (value) => {
  if (!value) return ''
  const stringValue = String(value)
  return stringValue.charAt(0).toUpperCase() + stringValue.slice(1).replaceAll('_', ' ')
}

const normalizeStatus = (status) => String(status || 'pending').toLowerCase().replaceAll(' ', '-').replaceAll('_', '-')
const handleImageError = (event) => event.target.classList.add('po-image-error')

onMounted(fetchCompletedOrders)
</script>
