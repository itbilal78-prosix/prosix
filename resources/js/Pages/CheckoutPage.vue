          ...
          <transition name="fade-up" mode="out-in">
            ...
            <div class="terms-agreement">
              ...
              <label class="terms-check-row">
                ...
                <input :checked="termsAccepted" type="checkbox" class="terms-checkbox" @click.prevent="openTerms" />
...
<button type="button" class="terms-link" @click.prevent="openTerms">
...
<span class="item-price">
  ${{ (Number(item.price) * item.quantity).toFixed(2) }}
    ...
    <!-- PROSIX SPORTS CUSTOM ORDER TERMS -->
    <transition name="terms-fade">
      <div v-if="showTerms" class="terms-overlay" @click.self="closeTerms">
        <div class="terms-book" role="dialog" aria-modal="true" aria-labelledby="terms-title">

          <div class="terms-book-header">
            <div class="terms-brand">
              <div class="terms-brand-mark">P</div>
              <div>
                <span class="terms-kicker">PROSIX SPORTS</span>
                <h2 id="terms-title">Custom Order Terms & Conditions</h2>
                <p>Please review all four pages before accepting.</p>
              </div>
            </div>
            <button type="button" class="terms-close" @click="closeTerms">×</button>
          </div>

          <div class="terms-progress">
            <button v-for="page in 4"
                    :key="page"
                    type="button"
                    :class="['terms-progress-item', { active: termsPage === page, done: termsPage > page }]"
                    @click="termsPage = page">
              <span>{{ page }}</span>
              <small>{{ termsPageTitles[page - 1] }}</small>
            </button>
          </div>
          ...
          <div class="terms-scroll">
            ...
            <!-- PAGE 1 -->
            <section v-if="termsPage === 1" class="terms-page">
              <div class="terms-page-head">
                <small>PAGE 1 OF 4</small>
                <h3>Order Details, Artwork & Customer Approval</h3>
                <p>These terms apply to custom uniforms, teamwear, apparel and other made-to-order products purchased from Prosix Sports.</p>
              </div>

              <div class="terms-highlight">
                <strong>Important: Custom-made products</strong>
                <p>Custom products are created specifically for your team or organization and may require design preparation, material sourcing, printing, sublimation, embroidery, cutting, sewing, names, numbers, logos, patches and other personalization.</p>
              </div>

              <article class="terms-clause">
                <h4>1. Customer order information</h4>
                <p>The customer is responsible for reviewing product style, quantity, size, player name, jersey number, spelling, team name, colors, logos, decoration positions, shipping information and special instructions before the order is submitted.</p>
              </article>

              <article class="terms-clause">
                <h4>2. Artwork and mockup approval</h4>
                <p>A digital proof, mockup or approved design is used as the production reference. Approval confirms that the customer has reviewed the layout and supplied information and authorizes Prosix Sports to prepare the order using those approved details.</p>
              </article>

              <article class="terms-clause">
                <h4>3. Names, numbers and spelling</h4>
                <p>Personalized text is produced from information submitted or approved by the customer. The customer must verify all names, numbers and spelling before production. An error already present in customer-approved information is not automatically a production error.</p>
              </article>

              <article class="terms-clause">
                <h4>4. Digital design representation</h4>
                <p>Digital previews are production guides. Screen calibration, fabric texture, print method, embroidery thread, lighting, material batches and ordinary manufacturing tolerances can cause reasonable differences between a digital preview and the finished item.</p>
              </article>
            </section>

            <!-- PAGE 2 -->
            <section v-else-if="termsPage === 2" class="terms-page">
              <div class="terms-page-head">
                <small>PAGE 2 OF 4</small>
                <h3>Production, Materials, Changes & Cancellation</h3>
                <p>Custom production creates real costs before a finished garment is ready.</p>
              </div>

              <article class="terms-clause">
                <h4>5. Production authorization</h4>
                <p>After required approvals and payment requirements are satisfied, Prosix Sports may purchase or reserve materials, prepare print files, schedule production, print or sublimate panels, embroider, cut fabric, sew garments, apply names and numbers, or complete other manufacturing work.</p>
              </article>

              <article class="terms-clause">
                <h4>6. Changes requested after approval</h4>
                <p>Changes after approval are not guaranteed. If a requested change is still possible, it may create additional charges and may extend the estimated completion date. Costs already incurred for materials or work that cannot reasonably be reused remain the customer's responsibility.</p>
              </article>

              <article class="terms-clause">
                <h4>7. Cancellation before custom work begins</h4>
                <p>A cancellation request received before materials are committed and before production begins may be reviewed. Design, setup, payment-processing, administrative or other non-recoverable costs already incurred may be deducted from an amount that is otherwise refundable.</p>
              </article>

              <article class="terms-clause warning">
                <h4>8. Cancellation after materials or production begin</h4>
                <p>Once custom materials have been purchased or committed, or printing, sublimation, embroidery, cutting, sewing, numbering, personalization or other custom production has started, the order is generally treated as committed custom production and may not be cancellable. Amounts committed to that custom production may be non-refundable, except where applicable law requires otherwise.</p>
              </article>

              <article class="terms-clause">
                <h4>9. Payment and remaining balance</h4>
                <p>The customer agrees to pay the amounts shown on the accepted checkout order, quote or invoice. Where a deposit or partial-payment arrangement is used, any remaining balance is due according to the agreed payment terms. Missing or late payment may delay production or shipment.</p>
              </article>
            </section>

            <!-- PAGE 3 -->
            <section v-else-if="termsPage === 3" class="terms-page">
              <div class="terms-page-head">
                <small>PAGE 3 OF 4</small>
                <h3>Sizing, Colors, Production Time & Delivery</h3>
                <p>Please review these practical terms carefully for team and roster orders.</p>
              </div>

              <article class="terms-clause">
                <h4>10. Sizing responsibility</h4>
                <p>The customer is responsible for selecting and submitting required sizes. Size charts or sample-sizing information should be reviewed before approval. A garment correctly produced to the ordered size is not considered a production error solely because that selected size does not fit the intended wearer.</p>
              </article>

              <article class="terms-clause">
                <h4>11. Color and material variation</h4>
                <p>Reasonable variation can occur between screen colors, separate production runs, fabric lots, printed panels, thread and finished garments. A reorder placed later may not be an exact shade match to an earlier production batch.</p>
              </article>

              <article class="terms-clause">
                <h4>12. Production and delivery estimates</h4>
                <p>Production and delivery dates are estimates unless expressly confirmed otherwise in writing. Timelines may be affected by approval delays, roster changes, payment delays, material availability, production capacity, carriers, weather, customs or other circumstances outside reasonable control.</p>
              </article>

              <article class="terms-clause">
                <h4>13. Shipping information</h4>
                <p>The customer must provide a complete and accurate delivery address. Additional carrier charges caused by an incorrect or incomplete address, failed delivery, re-routing or a customer-requested address change may be charged where applicable.</p>
              </article>

              <article class="terms-clause">
                <h4>14. Inspection after delivery</h4>
                <p>The shipment should be inspected promptly. Suspected shortages, wrong items, shipping damage or production issues should be reported as soon as reasonably possible. Photos, packaging information and order details may be requested to review the issue accurately.</p>
              </article>
            </section>

            <!-- PAGE 4 -->
            <section v-else class="terms-page">
              <div class="terms-page-head">
                <small>PAGE 4 OF 4</small>
                <h3>Production Issues, Remedies & Final Acceptance</h3>
                <p>This page explains the final order record and customer acknowledgment.</p>
              </div>

              <article class="terms-clause">
                <h4>15. Verified Prosix Sports production error</h4>
                <p>If a delivered custom item materially differs from the final approved order specifications because of a verified Prosix Sports production error, Prosix Sports will review the affected item and determine an appropriate remedy. Depending on the circumstances, this may include correction, repair, replacement, remake, credit or another mutually agreed resolution.</p>
              </article>

              <article class="terms-clause">
                <h4>16. Issues not caused by a production error</h4>
                <p>Customer-approved spelling, submitted sizes, approved artwork, customer-supplied logos or information, reasonable color variation, ordinary manufacturing tolerances, misuse, alterations after delivery or damage occurring after delivery are not automatically treated as Prosix Sports production errors.</p>
              </article>

              <article class="terms-clause">
                <h4>17. Order and acceptance records</h4>
                <p>Prosix Sports may retain the order number, approved design information, checkout details, acceptance date/time and the version of these terms as part of the business order record so both parties can identify the specifications and terms associated with the order.</p>
              </article>

              <article class="terms-clause">
                <h4>18. Applicable customer rights</h4>
                <p>Nothing in these terms is intended to remove or limit a customer right or remedy that cannot legally be waived under applicable law. If one provision is found unenforceable, the remaining provisions are intended to continue to apply to the extent permitted by law.</p>
              </article>

              <div class="terms-final">
                <div class="terms-final-icon">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
                <div>
                  <strong>Final customer acknowledgment</strong>
                  <p>By clicking <b>I Have Read & Agree</b>, I confirm that I have reviewed all four pages, understand that my order contains custom-made products, and accept these Custom Order Terms & Conditions for this order.</p>
                </div>
              </div>

              <p class="terms-disclaimer">These are general business terms. Prosix Sports should have the final wording reviewed for the jurisdictions in which it sells before relying on it as legal advice.</p>
            </section>
          </div>

          <div class="terms-footer">
            <button type="button" class="terms-back-btn" @click="termsPage > 1 ? prevTermsPage() : closeTerms()">
              {{ termsPage > 1 ? '← Previous' : 'Close' }}
            </button>

            <span class="terms-page-count">Page <b>{{ termsPage }}</b> of 4</span>

            <button v-if="termsPage < 4" type="button" class="terms-next-btn" @click="nextTermsPage">
              Next Page →
            </button>

            <button v-else type="button" class="terms-agree-btn" @click="acceptTerms">
...
const termsPage          = ref(1)
const termsPageTitles    = ['Order & Artwork', 'Production', 'Delivery', 'Acceptance']
...
const nextTermsPage = () => {
  if (termsPage.value < 4) termsPage.value++
}

const prevTermsPage = () => {
  if (termsPage.value > 1) termsPage.value--
}
...
/* ===== 4-PAGE CUSTOM ORDER TERMS POPUP ===== */
.terms-overlay{position:fixed;inset:0;z-index:999999;display:flex;align-items:center;justify-content:center;padding:22px;background:rgba(8,8,10,.74);backdrop-filter:blur(8px)}
.terms-book{width:min(900px,96vw);height:min(790px,93vh);display:flex;flex-direction:column;overflow:hidden;background:#fff;border-radius:20px;box-shadow:0 30px 100px rgba(0,0,0,.36)}
.terms-book-header{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:21px 24px 17px;border-bottom:1px solid #ececef}.terms-brand{display:flex;align-items:center;gap:12px}.terms-brand-mark{width:42px;height:42px;display:grid;place-items:center;flex:0 0 42px;border-radius:11px;background:#111;color:#fff;font-size:20px;font-weight:900;font-style:italic}
.terms-kicker{display:block;margin-bottom:3px;color:#8e8e93;font-size:9px;font-weight:800;letter-spacing:1.7px}.terms-book-header h2{margin:0;font-size:20px;font-weight:800;color:#111}.terms-book-header p{margin:4px 0 0;color:#8e8e93;font-size:11px}
.terms-close{width:36px;height:36px;border:1px solid #e5e5e8;border-radius:9px;background:#f7f7f8;color:#555;font-size:24px;line-height:1;cursor:pointer}.terms-close:hover{background:#111;color:#fff;border-color:#111}
.terms-progress{display:grid;grid-template-columns:repeat(4,1fr);background:#fafafa;border-bottom:1px solid #ececef}.terms-progress-item{min-height:52px;display:flex;align-items:center;justify-content:center;gap:8px;padding:8px;border:0;border-right:1px solid #ececef;background:transparent;color:#8e8e93;cursor:pointer}.terms-progress-item:last-child{border-right:0}.terms-progress-item span{width:24px;height:24px;display:grid;place-items:center;border:1px solid #d8d8dc;border-radius:50%;background:#fff;font-size:10px;font-weight:800}.terms-progress-item small{font-size:9px;font-weight:700}.terms-progress-item.active{background:#fff;color:#111}.terms-progress-item.active span,.terms-progress-item.done span{background:#111;border-color:#111;color:#fff}
.terms-scroll{flex:1;overflow-y:auto;padding:24px 28px 28px;overscroll-behavior:contain}.terms-page{max-width:760px;margin:0 auto}.terms-page-head{padding-bottom:16px;margin-bottom:5px;border-bottom:1px solid #ececef}.terms-page-head small{display:block;margin-bottom:6px;color:#8e8e93;font-family:'DM Mono',monospace;font-size:9px;font-weight:700;letter-spacing:1px}.terms-page-head h3{margin:0 0 7px;font-size:19px;font-weight:800;line-height:1.3;color:#111}.terms-page-head p{margin:0;color:#6d6d73;font-size:11.5px;line-height:1.7}
.terms-highlight{margin:16px 0 6px;padding:14px 16px;border:1px solid #dddde1;border-left:4px solid #111;border-radius:10px;background:#f7f7f8}.terms-highlight strong,.terms-final strong{display:block;margin-bottom:4px;color:#111;font-size:12px;font-weight:800}.terms-highlight p,.terms-final p{margin:0;color:#5f5f66;font-size:10.8px;line-height:1.7}
.terms-clause{padding:14px 0;border-bottom:1px solid #eeeef1}.terms-clause h4{margin:0 0 5px;color:#171719;font-size:12px;font-weight:800}.terms-clause p{margin:0;color:#5f5f66;font-size:10.8px;line-height:1.72}.terms-clause.warning{margin:10px 0;padding:14px 15px;border:1px solid #f1d5d5;border-radius:10px;background:#fff8f8}.terms-clause.warning h4{color:#9f1d1d}
.terms-final{display:flex;align-items:flex-start;gap:12px;margin-top:18px;padding:15px 16px;border:1.5px solid #d9d9dd;border-radius:12px;background:#f7f7f8}.terms-final-icon{width:38px;height:38px;flex:0 0 38px;display:grid;place-items:center;border-radius:10px;background:#111;color:#fff}.terms-disclaimer{margin:12px 2px 0;color:#96969c;font-size:9px;line-height:1.55}
.terms-footer{min-height:68px;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:12px;padding:12px 22px;border-top:1px solid #e8e8eb;background:#fafafa}.terms-page-count{color:#8e8e93;font-size:10px;text-align:center}.terms-page-count b{color:#111}.terms-back-btn,.terms-next-btn,.terms-agree-btn{min-height:42px;padding:0 17px;border-radius:9px;font-family:'Sora',sans-serif;font-size:11px;font-weight:750;cursor:pointer}.terms-back-btn{justify-self:start;border:1px solid #dedee2;background:#fff;color:#555}.terms-next-btn,.terms-agree-btn{justify-self:end;border:1px solid #111;background:#111;color:#fff}.terms-agree-btn{display:inline-flex;align-items:center;gap:7px}.terms-next-btn:hover,.terms-agree-btn:hover{background:#2a2a2a}
.terms-link{display:inline;padding:0;border:0;background:transparent;color:#111;font:inherit;font-weight:800;text-decoration:underline;text-underline-offset:3px;cursor:pointer}.terms-fade-enter-active,.terms-fade-leave-active{transition:opacity .18s ease}.terms-fade-enter-from,.terms-fade-leave-to{opacity:0}
@media(max-width:680px){.terms-overlay{align-items:flex-end;padding:8px}.terms-book{width:100%;height:95vh;border-radius:18px 18px 10px 10px}.terms-book-header{padding:16px}.terms-brand-mark{width:36px;height:36px;flex-basis:36px;font-size:17px}.terms-book-header h2{font-size:16px}.terms-progress-item{min-height:44px;padding:6px 3px}.terms-progress-item small{display:none}.terms-scroll{padding:18px 16px 22px}.terms-page-head h3{font-size:17px}.terms-footer{grid-template-columns:1fr 1fr;padding:10px 12px}.terms-page-count{display:none}.terms-back-btn,.terms-next-btn,.terms-agree-btn{width:100%;justify-content:center}}
...
/* =========================================
   TERMS POPUP — BIGGER & CLEARER TEXT
   ========================================= */

.terms-book-header h2 {
  font-size: 28px;
  font-weight: 800;
  line-height: 1.3;
}
...
/* Top 1,2,3,4 navigation */
.terms-progress-item span {
  font-size: 16px;
  font-weight: 800;
}

.terms-progress-item small {
  font-size: 15px;
  font-weight: 700;
}
