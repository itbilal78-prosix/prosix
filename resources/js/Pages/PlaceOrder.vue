...
/>
  ...
  Please select color
      ...
      <div class="po-mid-row">
        ...
        <!-- Quote/Invoice -->
        <div class="po-upload-card">
          <h6 class="po-upload-title">Your Qout/ Inv (Optional)</h6>
...
const orderNum = `P6S: ${year}-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    return {
      ...
      defaultSwatchColors: ['#000000', '#e63946', '#f4a261', '#2dc653', '#4895ef'],
...
this.showHelpSidebar = !this.showHelpSidebar;
    ...
    resetForm() {
      ...
      const today = new Date();
      this.orderNumber = 'ORD-' + today.getFullYear().toString().slice(-2)
        + String(today.getMonth()+1).padStart(2,'0')
        + String(today.getDate()).padStart(2,'0')
        + '-' + Math.floor(1000 + Math.random() * 9000);
    },
