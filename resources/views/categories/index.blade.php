                    ...
                    <tbody id="sortable">
                        @forelse($categories->whereNull('parent_id') as $cat)
                            ...
                            <tr class="parent-row" data-id="{{ $cat->id }}">
                                ...
                                <td>
                                    @if ($cat->icon_image)
                                        <img src="{{ $cat->icon_image }}" width="42" height="42"
                            ...
                            {{-- SUB CATEGORIES --}}
                            @foreach ($cat->subcategories()->orderBy('position')->get() as $sub)
                                <tr class="sub-row draggable-sub sub-of-{{ $cat->id }}" data-id="{{ $sub->id }}">
                                    <td colspan="5">
...
new Sortable(tbody,{
    animation:150,
    draggable:'.parent-row,.draggable-sub',

    onEnd:function(){

        let order=[];

        tbody.querySelectorAll('.parent-row,.draggable-sub').forEach((row,i)=>{
            order.push({
                id:row.dataset.id,
                position:i+1
            });
