...
<div class="container mt-4 d-flex justify-content-center">
    ...
    <div class="card shadow-sm p-4" style="max-width: 500px; width: 100%; border-radius: 10px;">
        ...
        <h2 class="text-center mb-4">{{ isset($font) ? 'Edit Font' : 'Add Font' }}</h2>

        <form action="{{ isset($font) ? route('fonts.update', $font->id) : route('fonts.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($font))
                @method('PUT')
            @endif
            ...
            <div class="mb-3">
                ...
                <div class="file-upload-box" onclick="document.getElementById('fileInput').click()">
                    <span id="file-name">{{ isset($font) && $font->file ? basename($font->file) : 'Click to upload font file' }}</span>
                    <input type="file" name="file" id="fileInput" accept=".ttf,.otf,.woff,.woff2">
                </div>

                @if(isset($font) && $font->file)
                    <small class="d-block mt-1">Current: <a href="{{ asset('storage/' . $font->file) }}" target="_blank">View</a></small>
                @endif
            </div>

            {{-- Submit Button --}}
            <button type="submit" class="btn btn-success w-100 font-btn">{{ isset($font) ? 'Update' : 'Add' }}</button>
