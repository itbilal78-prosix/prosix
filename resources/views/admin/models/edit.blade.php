    ...
    <div class="pf-grid">
      ...
      <div class="pf-card">
        ...
        <div class="pf-group">
          ...
          <input type="text" name="model_name" class="pf-input"
        ...
        <div class="pf-row-2">
          <div class="pf-group">
            ...
            <input type="number" step="0.01" min="0" name="price" class="pf-input"
              value="{{ old('price', $model->price) }}" placeholder="0.00">
          </div>
        </div>
        <div class="pf-group">
          <label class="pf-label">Description <small>(optional)</small></label>
          <textarea name="description" class="pf-textarea"
            placeholder="Model description...">{{ old('description', $model->description) }}</textarea>
        </div>
      </div>
      ...
      <div class="pf-card">
        ...
        <div class="pf-group" id="subcategoryWrapper">
      ...
      <div class="pf-card">
        ...
        <div class="pf-toggle-row">
          <div class="pf-toggle-info">
            ...
            <span>Model available for purchase</span>
          </div>
          <label class="pf-toggle">
            <input type="checkbox" name="in_stock" value="1"
              {{ old('in_stock', $model->in_stock) ? 'checked' : '' }}>
        ...
        <div style="margin-top:14px">
          <div class="pf-group">
            ...
            <input type="number" min="0" name="stock_quantity" class="pf-input"
              value="{{ old('stock_quantity', $model->stock_quantity) }}"
              placeholder="Leave empty = unlimited" style="max-width:220px">
      ...
      <div class="pf-card">
        ...
        <div class="pf-toggle-row">
          <div class="pf-toggle-info">
            ...
            <span>This model requires shipping</span>
          </div>
          <label class="pf-toggle">
            <input type="checkbox" name="shipping_enabled" id="shippingToggle" value="1"
              {{ old('shipping_enabled', $model->shipping_enabled) ? 'checked' : '' }}>
            <span class="pf-toggle-slider"></span>
          </label>
        </div>
        <div id="shippingFields" style="margin-top:14px">
          <div class="pf-row-2">
            <div class="pf-group">
              ...
              <input type="number" step="0.01" min="0" name="shipping_cost" class="pf-input"
                value="{{ old('shipping_cost', $model->shipping_cost ?? '0') }}" placeholder="0.00">
              <span class="pf-hint">0 = Free shipping</span>
            </div>
            <div class="pf-group">
              <label class="pf-label">Free Above ($) <small>(optional)</small></label>
              <input type="number" step="0.01" min="0" name="free_shipping_above" class="pf-input"
                value="{{ old('free_shipping_above', $model->free_shipping_above) }}" placeholder="e.g. 200">
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="pf-card">
        ...
        @if($model->size_chart_image)
          <p style="font-size:11px;color:#555;margin-bottom:8px">
            Current: <strong>{{ basename($model->size_chart_image) }}</strong>
          </p>
          <div style="margin-bottom:12px">
            <img src="{{ asset('storage/' . $model->size_chart_image) }}"
              style="max-width:240px;max-height:140px;object-fit:contain;border:1px solid #e0e0e0;border-radius:8px;display:block">
          </div>
          <label style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;cursor:pointer;margin-bottom:12px">
            <input type="checkbox" name="remove_size_chart" value="1" id="removeSizeChart">
            Remove current size chart
          </label>
        @endif

        <div class="sc-box {{ $model->size_chart_image ? '' : '' }}" id="scBox"
          onclick="document.getElementById('scInput').click()">
          <div class="upload-placeholder">
            <i class="bi bi-cloud-upload" style="font-size:26px;color:#ccc;display:block;margin-bottom:6px"></i>
            <p style="font-size:11px;color:#aaa;margin:0">
              {{ $model->size_chart_image ? 'Click to replace size chart' : 'Click to upload size chart' }}
            </p>
          </div>
          <img id="scPreview" src="" alt="">
        </div>
        <div id="scActions" style="display:none;margin-top:8px;gap:8px" class="d-flex">
          <button type="button" class="btn-back" style="height:36px;padding:0 16px;font-size:11px"
            onclick="document.getElementById('scInput').click()">
            <i class="bi bi-arrow-repeat"></i> Replace
          </button>
          <button type="button" class="btn-back" style="height:36px;padding:0 16px;font-size:11px;background:#000;color:#fff"
    ...
    <div class="pf-grid-full">
      <div class="pf-card">
        ...
        <p style="font-size:11px;color:#888;margin-bottom:16px">
          Upload front, back, left, right views in black, white, and SVG formats.
        </p>
          ...
          <div class="d-flex flex-wrap gap-2">
            ...
            @foreach($views as $view)
              @foreach($colors as $color)
                ...
                @endphp
                <div class="image-box">
                  <label class="pf-label" style="margin-bottom:4px;text-transform:capitalize">
                    {{ $view }} {{ $color }}
                  </label>
                  ...
                  <div class="img-preview {{ $imagePath ? '' : 'empty-preview' }}"
                    ...
                    @if($imagePath)
                      ...
                      <div class="close-btn" onclick="removeImage('{{ $view }}','{{ $color }}')">×</div>
                    @else
                      <span>Click</span>
                    @endif
                  </div>
          ...
          <div style="min-width:220px">
            <div style="border:1.5px solid #e8e8e8;border-radius:12px;padding:16px;text-align:center">
              ...
              <div class="thumbnail-3d mb-2" id="thumbnail-3d">
                @foreach($colors as $color)
                  @php $frontFile = $model->{'front_' . $color} ?? ''; @endphp
              ...
              <div style="display:flex;justify-content:space-between;gap:8px;margin-top:10px">
                <button type="button" class="btn-back" style="height:34px;padding:0 14px;font-size:11px"
                  onclick="prevView()">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function loadSubcategories(selectedSubId) {
    ...
    if (found && found.subcategories && found.subcategories.length) {
      found.subcategories.forEach(sub => {
        const opt = document.createElement('option');
        opt.value = sub.id;
        opt.textContent = sub.name;
        if (sub.id == (selectedSubId ?? {{ old('subcategory_id', $model->subcategory_id ?? 'null') }})) {
          opt.selected = true;
        }
  ...
  catSelect.addEventListener('change', () => loadSubcategories(null));
  ...
  // Load subcategories on page load (edit mode)
  loadSubcategories({{ old('subcategory_id', $model->subcategory_id ?? 'null') }});
  ...
  // ── Sizes ────────────────────────────────────────────────────
  // Pre-populate from saved sizes
  const savedSizes = @json(old('sizes', $model->sizes ?? []));
  let selectedSizes = Array.isArray(savedSizes) ? [...savedSizes] : [];

  // Sync checkboxes with saved sizes
  document.querySelectorAll('.size-checkbox').forEach(cb => {
    if (cb.value !== 'All' && selectedSizes.includes(cb.value)) {
      cb.checked = true;
    }
  });
  renderSizeTags();
  ...
  document.getElementById('sizesTrigger').addEventListener('click', function () {
    const d = document.getElementById('sizesDropdown');
    d.classList.toggle('open');
    document.getElementById('sizesChevron').className = d.classList.contains('open')
    ...
    cb.addEventListener('change', function () {
      ...
      if (this.value === 'All') {
        ...
        if (this.checked) { if (!selectedSizes.includes(this.value)) selectedSizes.push(this.value); }
          ...
          selectedSizes = selectedSizes.filter(s => s !== this.value);
          document.getElementById('sz_All').checked = false;
        }
      }
      renderSizeTags();
    });
  ...
  function renderSizeTags() {
    ...
    selectedSizes.forEach(sz => {
      ...
      t.innerHTML = `${sz} <span class="size-tag-cross" onclick="removeSize('${sz}')">×</span>`;
      tags.appendChild(t);
      const i = document.createElement('input'); i.type = 'hidden'; i.name = 'sizes[]'; i.value = sz;
      hid.appendChild(i);
    });
  }
  ...
  window.removeImage = function (view, type) {
    ...
    if (view === 'front') {
      const t = document.getElementById('p_' + type);
      if (t) t.src = existingImages['front'][type] || '';
    }
  };
  ...
  window.showThumbnail = function (viewIndex) {
    ...
    ['black', 'white', 'svg'].forEach(color => {
      ...
      if (input && input.files && input.files[0]) {
        ...
        if (img) img.src = existingImages[view][color] || '';
      }
    });
  };
