        ...
        <div style="flex:1; overflow:hidden; display:flex;">
            ...
            <div id="msContent2" style="display:none; width:100%; height:100%; overflow:hidden; flex-direction:column;">
                <div style="flex:1; overflow:hidden; position:relative;">
                    <iframe id="mascotCreatorFrame"
                        src=""
                        style="width:100%; height:100%; border:none; display:block;"
                        title="Mascot Creator">
                    </iframe>
                    <div id="mascotCreatorLoading"
                        ...
                        <div style="width:40px;height:40px;border:4px solid #eee;border-top-color:#1a1a1a;border-radius:50%;animation:msSpinAnim 0.8s linear infinite;"></div>
                        <p style="color:#888;font-size:13px;margin:0;">Loading mascot creator...</p>
                    </div>
                </div>
                <div style="padding:12px 20px;border-top:1px solid #eee;background:#fafafa;display:flex;gap:10px;align-items:center;flex-shrink:0;">
                    <span style="font-size:13px;color:#666;flex:1;">Create your mascot above, then click <strong>Use This Mascot</strong> to apply it.</span>
            ...
            <div id="msContent3" style="display:none; width:100%; height:100%; overflow-y:auto; padding:32px;">
                <div style="max-width:560px; margin:0 auto;">
                    ...
                    <p style="color:#777; font-size:13px; margin-bottom:28px;">Upload an SVG, PNG or JPG file from your computer.</p>
...
<div id="mascotCustomizeModal"
    ...
    <div style="background:#fff; width:900px; max-width:95vw; max-height:90vh; border-radius:12px; overflow:hidden; display:flex; flex-direction:column; box-shadow:0 24px 80px rgba(0,0,0,0.5);">

        {{-- Header --}}
        <div style="padding:16px 24px; background:#1a1a1a; color:#fff; display:flex; justify-content:space-between; align-items:center; flex-shrink:0;">
            <h3 style="margin:0; font-size:16px; font-weight:700; letter-spacing:1px;">CUSTOMIZE MASCOT</h3>
            <span onclick="closeMascotCustomizeModal()" style="cursor:pointer; font-size:26px; line-height:1; opacity:.8;" onmouseenter="this.style.opacity=1" onmouseleave="this.style.opacity=.8">×</span>
        </div>
        ...
        <div style="display:flex; flex:1; overflow:hidden;">
            ...
            <div style="flex:1; display:flex; flex-direction:column; border-right:1px solid #eee;">
                {{-- Toolbar --}}
                <div style="padding:10px 16px; border-bottom:1px solid #eee; display:flex; gap:10px; align-items:center; flex-shrink:0; background:#fafafa;">
                    ...
                    <div style="display:flex;align-items:center;gap:6px;" id="mcEraserSizeWrap" style="display:none;">
                        <label style="font-size:11px;color:#666;">Size:</label>
                        <input type="range" id="mcEraserSize" min="5" max="60" value="20"
                            style="width:80px;" oninput="document.getElementById('mcEraserSizeVal').textContent=this.value">
                ...
                {{-- Canvas --}}
                <div style="flex:1; display:flex; align-items:center; justify-content:center; background:#e8e8e8; position:relative; overflow:hidden;">
                    <canvas id="mcCanvas" style="background: repeating-conic-gradient(#ccc 0% 25%, #fff 0% 50%) 0 0 / 16px 16px; border-radius:4px; cursor:crosshair;"></canvas>
                </div>
            </div>
            ...
            <div style="width:260px; overflow-y:auto; flex-shrink:0;">
                ...
                <div style="padding:16px; border-bottom:1px solid #eee;">
                    <label style="font-size:11px; font-weight:700; color:#999; letter-spacing:1px; display:block; margin-bottom:6px;">MASCOT NAME</label>
                ...
                <div style="padding:16px; border-bottom:1px solid #eee;">
                    <label style="font-size:11px; font-weight:700; color:#999; letter-spacing:1px; display:block; margin-bottom:8px;">COLORS IN MASCOT</label>
                    <div style="display:flex; flex-wrap:wrap; gap:6px;" id="mcColorCountBtns">
                        <button onclick="mcSelectColorCount(1,this)" style="padding:5px 10px;border:2px solid #ddd;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;">1</button>
                        <button onclick="mcSelectColorCount(2,this)" style="padding:5px 10px;border:2px solid #1a1a1a;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#1a1a1a;color:#fff;">2</button>
                        <button onclick="mcSelectColorCount(3,this)" style="padding:5px 10px;border:2px solid #ddd;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;">3</button>
                        <button onclick="mcSelectColorCount(4,this)" style="padding:5px 10px;border:2px solid #ddd;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;">4</button>
                        <button onclick="mcSelectColorCount(5,this)" style="padding:5px 10px;border:2px solid #ddd;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;">5</button>
                        <button onclick="mcSelectColorCount(6,this)" style="padding:5px 10px;border:2px solid #ddd;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;">6</button>
                        <button onclick="mcSelectColorCount(7,this)" style="padding:5px 10px;border:2px solid #ddd;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;">7</button>
                        <button onclick="mcSelectColorCount(8,this)" style="padding:5px 10px;border:2px solid #ddd;border-radius:5px;font-size:12px;font-weight:700;cursor:pointer;background:#fff;">8+</button>
                    </div>
                    <p id="mcColorCountInfo" style="font-size:11px;color:#888;margin:8px 0 0;">Detected: <strong id="mcDetectedColors">0</strong> colors</p>
                </div>

                {{-- Color Replacement --}}
                <div style="padding:16px; border-bottom:1px solid #eee;">
                    <label style="font-size:11px; font-weight:700; color:#999; letter-spacing:1px; display:block; margin-bottom:10px;">CHANGE COLORS</label>
                ...
                {{-- Background --}}
                <div style="padding:16px; border-bottom:1px solid #eee;">
                    <label style="font-size:11px; font-weight:700; color:#999; letter-spacing:1px; display:block; margin-bottom:8px;">BACKGROUND</label>
                    ...
                    <p style="font-size:11px;color:#999;margin:6px 0 0;">Or use Eraser tool above to manually erase areas</p>
                </div>

                {{-- Opacity --}}
                <div style="padding:16px; border-bottom:1px solid #eee;">
                    <label style="font-size:11px; font-weight:700; color:#999; letter-spacing:1px; display:block; margin-bottom:8px;">OPACITY</label>
        ...
        {{-- Footer --}}
        <div style="padding:14px 24px; border-top:1px solid #e8e8e8; display:flex; justify-content:space-between; align-items:center; flex-shrink:0; background:#fafafa;">
...
window.openMascotSelectModal = function(layerId) {
    ...
    if (!modal) { console.error('❌ mascotSelectModal not found'); return; }
...
window.switchMascotSelectTab = function(tab) {
    ...
    var tabIds  = { existing:'msTab1', create:'msTab2', upload:'msTab3' };
    var contIds = { existing:'msContent1', create:'msContent2', upload:'msContent3' };
...
window.importMascotFromCreator = function() {
    var frame = document.getElementById('mascotCreatorFrame');
    if (!frame || !frame.contentWindow) { alert('Creator not loaded'); return; }
    ...
    try {
        ...
        if (!svgStr) { alert('Could not get mascot from creator. Please save first.'); return; }
...
function _loadMascotTemplates() {
    ...
    fetch('/api/mascot-templates')
        ...
        .catch(function(){
            ...
            if (g) g.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;color:#aaa;font-size:13px;">Could not load mascots.</div>';
        });
}

function _buildCategoryList(mascots) {
    var categories = [];
    mascots.forEach(function(m){ if(m.category && categories.indexOf(m.category)===-1) categories.push(m.category); });
...
window.searchMascots = function(query) {
    ...
    _renderMascotGrid(q ? base.filter(function(m){ return m.title && m.title.toLowerCase().includes(q); }) : base);
};

function _renderMascotGrid(mascots) {
    var grid = document.getElementById('mascotSelectGrid');
    if (!grid) return;
    if (!mascots.length) { grid.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:40px;color:#aaa;font-size:13px;">No mascots found.</div>'; return; }
    grid.innerHTML = '';
...
window.applySelectedMascotToApplication = function() {
    ...
    if (!mascotData) { alert('Please select a mascot first.'); return; }

    var layerId = state.pendingLayerId || window.currentApplicationLayer;
    if (!layerId) { alert('No application layer found.'); return; }
...
window.openMascotCustomizeModal = function(mascotData, layerId) {
    ...
    setTimeout(function(){ mcInitCanvas(); }, 100);
};
...
function mcErase(canvas, e) { mcBrushErase(canvas, e); }
...
window.mcSetTool = function(tool) {
    ...
    if (tool === 'select') {
        if (selBtn) { selBtn.style.background='#1a1a1a'; selBtn.style.color='#fff'; selBtn.style.borderColor='#1a1a1a'; }
        if (erBtn)  { erBtn.style.background='#fff';     erBtn.style.color='#333'; erBtn.style.borderColor='#ccc'; }
        if (erWrap) erWrap.style.display = 'none';
        if (canvas) canvas.style.cursor = 'default';
    } else {
        if (erBtn)  { erBtn.style.background='#1a1a1a'; erBtn.style.color='#fff'; erBtn.style.borderColor='#1a1a1a'; }
        if (selBtn) { selBtn.style.background='#fff';    selBtn.style.color='#333'; selBtn.style.borderColor='#ccc'; }
...
document.addEventListener('DOMContentLoaded', function() {
    var erSize = document.getElementById('mcEraserSize');
    if (erSize) erSize.addEventListener('input', function(){ window._mc.eraserRadius = parseInt(this.value)||20; });
});
...
function mcDetectColors(ctx, w, h) {
    ...
    for (var i = 0; i < data.length; i += 16) {
        ...
        var hex = rgbToHex(Math.min(r,255), Math.min(g,255), Math.min(b,255));
        colorCounts[hex] = (colorCounts[hex] || 0) + 1;
    }

    // Sort by count, take top N
    var sorted = Object.entries(colorCounts).sort(function(a,b){ return b[1]-a[1]; });
    var topColors = sorted.slice(0, window._mc.colorCount).map(function(e){ return e[0]; });
...
window.mcSelectColorCount = function(n, btn) {
    ...
    if (btn) { btn.style.background='#1a1a1a'; btn.style.color='#fff'; btn.style.borderColor='#1a1a1a'; }
    var canvas = document.getElementById('mcCanvas');
    if (canvas) {
        var ctx = canvas.getContext('2d');
...
function mcRenderColorSwatches(detectedColors) {
    ...
    var backendColors = (window.backendColors || []).map(function(c){ return c.code || c.hex || c.color || c; });
    if (!backendColors.length) backendColors = ['#FF0000','#00FF00','#0000FF','#FFFFFF','#000000','#FFFF00','#FF00FF','#00FFFF'];
    ...
    detectedColors.forEach(function(detectedHex) {
        ...
        fromBox.style.cssText = 'width:28px;height:28px;border-radius:5px;border:2px solid #ccc;flex-shrink:0;';
        ...
        backendColors.slice(0, 12).forEach(function(hex) {
            ...
            box.style.cssText = 'width:22px;height:22px;border-radius:4px;cursor:pointer;border:'+(isSelected?'3px solid #1a1a1a':'2px solid #ddd')+';box-sizing:border-box;transition:border .1s;';
...
function mcApplyColorReplacements() {
    ...
    function _doReplace(imageData) {
        ...
        Object.entries(window._mc.colorMap).forEach(function(entry) {
            ...
            if (from && to) replacements.push({ from: from, to: to });
        });
...
window.mcRemoveBackground = function() {
    ...
    var corners = [0, (canvas.width-1)*4, (canvas.height-1)*canvas.width*4, ((canvas.height-1)*canvas.width + canvas.width-1)*4];
    var bgSamples = corners.map(function(i){ return {r:data[i], g:data[i+1], b:data[i+2]}; });
    var bgR = Math.round(bgSamples.reduce(function(s,c){return s+c.r;},0)/4);
    var bgG = Math.round(bgSamples.reduce(function(s,c){return s+c.g;},0)/4);
    var bgB = Math.round(bgSamples.reduce(function(s,c){return s+c.b;},0)/4);
...
window.mcSaveAndApply = function(doSave) {
    ...
    // Wrap in SVG
    var finalSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+canvas.width+' '+canvas.height+'">'
                 + '<image href="'+dataUrl+'" x="0" y="0" width="'+canvas.width+'" height="'+canvas.height+'" preserveAspectRatio="xMidYMid meet" opacity="'+(window._mc.opacity/100)+'"/>'
                 + '</svg>';
    ...
    if (layer) layer.mascotTitle = name;
...
function _mcSaveToBackend(name, dataUrl, svgData) {
    ...
    fetch('/api/mascot-templates', {
        ...
        body: JSON.stringify({ title: name, svg_data: svgData, image_data: dataUrl, category: 'Custom' })
    })
    .then(function(r){ return r.json(); })
    .then(function(res){
        console.log('✅ Mascot saved:', res);
        // Reload mascot list silently
        fetch('/api/mascot-templates').then(function(r){ return r.json(); }).then(function(data){
            window._mascotState.allMascots    = data;
            window._mascotState.filteredMascots = data;
        });
    })
...
function _handleMascotFile(file) {
    ...
    reader.onload = function(e) {
        ...
        if (file.type === 'image/svg+xml') {
            if (thumb) { thumb.innerHTML = result; var s=thumb.querySelector('svg'); if(s){s.style.width='100%';s.style.height='100%';} }
            svgStr = result;
        } else {
...
window.clearMascotUpload = function() {
    ...
    if (inp) inp.value = '';
    if (zn) { zn.style.borderColor='#ccc'; zn.style.background='#fafafa'; }
};

window.applyUploadedMascotFile = function() {
    var data = window._mascotState.selectedMascotData;
    if (!data) { alert('Please select a file first.'); return; }
