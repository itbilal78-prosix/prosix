...
@section('content')
    ...
    <div class="container-fluid">
        ...
        @if (session('success'))
            <div class="alert alert-success fade show" id="successAlert">{{ session('success') }}</div>
        @endif
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.switchModelSubTab = function(btn, catId, subId) {
                ...
                document.querySelectorAll(
                        `.model-group-divider[data-cat="${catId}"],.model-group-row[data-cat="${catId}"]`)
                    .forEach(el => {
                        if (subId === 'all') {
                            el.style.display = '';
                            return;
            ...
            // ─── Render toolbar buttons based on selection ───
            function renderToolbar(blockId) {
                ...
                if (!allFeat) {
                    ...
                    b.className = 'btn btn-dark btn-sm';
                    b.innerHTML = `<i class="bi bi-star-fill me-1"></i> Add Featured (${n})`;
                    b.addEventListener('click', () => doAction('{{ route('models.featured') }}', 'add', blockId));
                    toolbar.appendChild(b);
                }
                if (!noneFeat) {
                    const b = document.createElement('button');
                    b.className = 'btn btn-outline-dark btn-sm';
                    b.innerHTML = `<i class="bi bi-star me-1"></i> Remove Featured (${n})`;
                    b.addEventListener('click', () => doAction('{{ route('models.featured') }}', 'remove',
                        blockId));
                    toolbar.appendChild(b);
                }
                ...
                if (!allApp) {
                    const b = document.createElement('button');
                    b.className = 'btn btn-dark btn-sm';
                    b.innerHTML = `<i class="bi bi-bag-fill me-1"></i> Add Apparel (${n})`;
                    b.addEventListener('click', () => doAction('{{ route('models.apparel') }}', 'add', blockId));
                    toolbar.appendChild(b);
                }
                if (!noneApp) {
                    const b = document.createElement('button');
                    b.className = 'btn btn-outline-dark btn-sm';
                    b.innerHTML = `<i class="bi bi-bag me-1"></i> Remove Apparel (${n})`;
                    b.addEventListener('click', () => doAction('{{ route('models.apparel') }}', 'remove',
                    blockId));
                    toolbar.appendChild(b);
                }
            ...
            // ─── Individual checkbox with shift-click ───
                ...
                cb.addEventListener('click', function(e) {
                    ...
                    if (e.shiftKey && lastChecked && lastChecked.dataset.block === blockId) {
                        const all = Array.from(document.querySelectorAll(
                                `.model-checkbox[data-block="${blockId}"]`))
            ...
            async function bulkDelete(blockId) {
                ...
                if (!confirm(
                        `Are you sure you want to delete ${ids.length} model(s)?\nThis action cannot be undone!`
                        )) return;
