                ...
                <tbody>
                    @foreach($deals as $deal)
                        <tr>
                            ...
                            <td class="text-black">{{ $deal->subtitle ?? '-' }}</td>
                            <td>
                                @if($deal->images->count())
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach($deal->images->take(3) as $img)
                                            <img src="{{ $img->image_path }}" class="img-thumbnail" style="width:60px; height:60px; object-fit:cover;">
                                        @endforeach
                                        @if($deal->images->count() > 3)
                                            <span class="badge bg-secondary">+{{ $deal->images->count() - 3 }}</span>
                                        @endif
                                    </div>
                                @else
                            ...
                            <td class="text-black">{{ $deal->button_text ?? '-' }}</td>
                            <td>
                                ...
                                <form action="{{ route('deals.destroy', $deal) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-dark" onclick="return confirm('Delete this deal?')">
...
.btn-dark { background-color: #000; color: #fff; border: 1px solid #000; transition: all 0.3s ease; }
.btn-dark:hover { background-color: #fff; color: #000; border-color: #000; }
.btn-outline-dark { color: #000; border: 1px solid #000; transition: all 0.3s ease; }
.btn-outline-dark:hover { background-color: #000; color: #fff; }
.img-thumbnail { border: 1px solid #000; border-radius: 8px; }
